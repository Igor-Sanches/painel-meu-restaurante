...
public class ViewLoadingDotsFade extends LinearLayout {
    ...
    public ViewLoadingDotsFade(Context context2) {
        super(context2);
    }

    public ViewLoadingDotsFade(Context context2, @Nullable AttributeSet attributeSet) {
    ...
    public ViewLoadingDotsFade(Context context2, @Nullable AttributeSet attributeSet, int i) {
    ...
    public void animateView(final boolean z) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            this.animator[i] = ObjectAnimator.ofPropertyValuesHolder(this.img[i], ofFloat);
            this.animator[i].setRepeatCount(0);
            this.animator[i].setRepeatMode(ValueAnimator.REVERSE);
            this.animator[i].setDuration(400);
            this.animator[i].setStartDelay(i * DURATION);
            this.animator[i].start();
        }
        this.animator[2].addListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator animator) {
                super.onAnimationEnd(animator);
                ViewLoadingDotsFade.this.animateView(!z);
            }
        });
    }
}
